...
#			$shit = `eyeD3 --remove-all \"$file\"`;
			$shit = `mid3v2 --delete-frames=TXXX:ALBUMARTISTSORT \"$file\"`;
			$shit = `eyeD3 --to-v2.4 --force-update --remove-v1 -a \"$artist\" -A \"$albumt\" -b \"$artist\" -t \"$song\" -n $tracknum --tagging-date $date --remove-frame TSOP --add-image=\"$albumdir/$album/cover.jpg\":FRONT_COVER \"$file\"`;
		    }
		}
	    }
	}
